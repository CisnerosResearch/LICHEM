...
def RecoverEnergy(txtLabel, itemNum):
    ...
    :param itemNu Column to text to interpret as the energy
...
if (allTests == 0):
    ...
    if ((QMPack == "psi4") or (QMPack == "psi")):
        ...
        try:
            ...
            QMbin = "N/A"
        badQM = 0
    elif ((QMPack == "gaussian") or (QMPack == "g09")):
        QMPack = "Gaussian"
        cmd = "which g09"
        try:
            # Find path
...
line += ' Total run time: {time:.2f}{units}'.format(time=round(totalTime, 2),
                                                    units=timeUnits)
line += """
