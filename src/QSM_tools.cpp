...
void ODESolve(VectorXd& path, MatrixXd& H, VectorXd& g, VectorXd& energy,
              int Nimages, int beadsize, VectorXd& trs, VectorXd& cons, 
              int wholesize, double ftol, double &dftot, VectorXd& weight,
              fstream& logFile)
{
...
void RKStep(VectorXd& path, VectorXd& epath, MatrixXd& H, VectorXd& g,
            VectorXd& energy, int Nimages, int beadsize, double h, 
            double error0, double &error1, VectorXd& x5, double &hnew,
            bool currswitch[], VectorXd& cons, VectorXd& weight)
{
  ...
  sumks = 1631.0/55296.0*ks.row(0) + 175.0/512.0*ks.row(1) +
          575.0/13824.0*ks.row(2) + 44275.0/110592.0*ks.row(3) +
          253/4096.0*ks.row(4);
  path1 = path + sumks;
...
void quad_app(VectorXd& wholepath, VectorXd& oldpath, MatrixXd& Hessmat, 
              VectorXd& force, VectorXd& energy, int Nimages, int beadsize, 
              VectorXd& equad, VectorXd& gquad)
{

  VectorXd dx((Nimages+2)*beadsize);
  // Hessian of current image
  MatrixXd Hc(beadsize,beadsize);
  // Difference between current and oldpath for the current image
  VectorXd dxc(beadsize);
  // Hessian*dxci
  VectorXd Hdx(beadsize);
  // Force of current image
  VectorXd Fc(beadsize);
  // Force of current image
  VectorXd gquadc(beadsize);

  // Dot product of hessian and path difference of the current image (Fc.dxc) 
  double Fcdx;
...
void calc_tangents(VectorXd& path, int Nimages, int beadsize, 
                  VectorXd& tangents, VectorXd& energy, VectorXd& weight,
                  int choice)
{
  ...
  else
  {
    ...
    for(int k=1; k<Nimages+1; k++)
    {
      ...
      /*
        currtan = currtan/currtan.norm();
        (sqrt((currtan.array().square()).sum()));
      */
...
void updateTR (MatrixXd& Hessmat, VectorXd& glast, VectorXd& oldpath, 
              ...
              VectorXd& lastenergy, VectorXd& trs, VectorXd& maxtr,
              int beadsize,int Nimages)
{
  ...
  VectorXd gc(beadsize);
  // Current and previous energy of the corresponding image
  double Ec, Ep;
  // Change in energy
  double Ediff;
  double newTR;
  ...
  for (int k=1; k<Nimages+1; k++)
  {
    ...
    if ( (abs(Ediff)>1e-10) and (dxc.norm() > 1e-7))
    {
      ...
      */
              ...
              (trs(k-1)<= (sqrt((dxc.array().square()).sum())*1.4)))
      {
      ...
      /*
        ...
        { 
      */
      else if ((rho < 0.85) and 
              ((sqrt((dxc.array().square()).sum())/trs(k-1))<0.8))
      { 
        newTR = trs(k-1)/2;
      }
      /* 
        ...
        { 
      */
      else if ((rho < 0.95) and 
              ((sqrt((dxc.array().square()).sum())/trs(k-1))<0.5))
      {
        newTR = trs(k-1)/2;
      }
    }
