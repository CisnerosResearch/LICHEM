...
int main(int argc, char* argv[])
{
  ...
  ReadLICHEMInput(xyzFile,connectFile,regionFile,
                  QMMMData,QMMMOpts,logFile,stat);
  if (stat!=0)
  {
    logFile.close();
    errFile.close();
  ...
  //     else-if statement.
  //     After the else-if, an "// End of section" comment should be added
  //     to mark where the next simulation type begins.
  ...
  if (SinglePoint)
  {
    ...
    for (int p=0;p<QMMMOpts.NBeads;p++)
    {
      ...
      if (QMMM or QMonly)
      {
        ...
        */
        // Hartree (a.u.)
        logFile << "  QM energy: " << LICHEMFormFloat(Eqm/har2eV,16);
        logFile << " a.u." << '\n';
        // Print progress
        logFile.flush();
        // End kcal
      ...
      if (QMMM or MMonly)
      {
        ...
        */
        // Hartree (a.u.)
        logFile << "  MM energy: " << LICHEMFormFloat(Emm/har2eV,16);
        logFile << " a.u." << '\n';
      }
  ...
  else if (OptSim)
  {
    ...
    while (!optDone)
    {
      ...
      // Check convergence
      optCt += 1;
      optDone = OptConverged(QMMMData,OldQMMMData,forces,optCt,
                             QMMMOpts,0,0,logFile);
    }
    logFile << '\n';
