...
void LICHEMQSMMPI(vector<QMMMAtom>& QMMMData, QMMMSettings& QMMMOpts, \
                  ...
                  VectorXd& force, double spaceout_dist, 
                  VectorXd& Eqmmm_images, int macroiter, fstream& logFile)
{

  // Generic file stream
  fstream ofile,qmfile,initfile,pathfile,hessfile,gfile,p0file;
  ...
  while ( (!PathDoneQM) and (qsmiter < maxiter))
  {
      ...
      else
      {
        ...
        if (primaryCPU)
        {
          ...
          for (int k=0; k<Nimages; k++)
          {
            ...
            /*
              ...
              // RMS force
              ForceStats(k+1,1) = (gtan_curr).squaredNorm();
            */
            ForceStats(k+1,0) = MAXforce*bohrRad;
            // RMS force
            ForceStats(k+1,1) = (gtan_curr*bohrRad).squaredNorm();
        ...
        MPI_Bcast(&max_dfval,1,MPI_DOUBLE,root,MPI_COMM_WORLD);
                                                            /* group_comm); */
          ...
          if (primaryCPU)
          {
            ...
            if (QMMMOpts.debug)
            {
              for (int k = 1; k < Nimages+1; k++)
              {
                ...
                int csize=beadsize;  // Nnumber of columns
