      ...
      if (receiver!=root) // Start sending
      {
        ...
        if (master)
        {
          ...
          /*
            if (i==1)
            {
              cout << "Rec:" << receiver << " b:" << j;
              cout << " m:"<< QMMMData[i].m <<" ...\n" << endl;
            }
          */

          // Bools
          MPI_Send(&QMMMData[i].NEBActive,1,MPI::BOOL,
                    receiver,103,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].QMRegion,1,MPI::BOOL,
                    receiver,104,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].MMRegion,1,MPI::BOOL,
                    receiver,105,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].PBRegion,1,MPI::BOOL,
                    receiver,106,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].BARegion,1,MPI::BOOL,
                    receiver,107,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].frozen,1,MPI::BOOL,
                    receiver,108,MPI_COMM_WORLD);

          // Integers
          MPI_Send(&QMMMData[i].numTyp,1,MPI_INT,
                    receiver,109,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].numClass,1,MPI_INT,
                    receiver,110,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].id,1,MPI_INT,
          ...
          MPI_Send(&QMMMData[i].QMTyp[0],sz1+1,MPI_CHAR,
                    receiver,114,MPI_COMM_WORLD);
          MPI_Send(&QMMMData[i].MMTyp[0],sz2+1,MPI_CHAR,
                    receiver,115,MPI_COMM_WORLD);
          ...
          // Bond info
          MPI_Send(&QMMMData[i].bonds[0],bsz,MPI_INT,
                    receiver,121,MPI_COMM_WORLD);
          ...
          if (sz3!=0)
          {
            // In the test case sz3 was 0 so it was giving an error
        ...
        if ((myrank==receiver))
        {
          ...
          */
          ...
          // Bools
          MPI_Recv(&QMMMData[i].NEBActive,1,MPI::BOOL,
                    root,103,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].QMRegion,1,MPI::BOOL,
                    root,104,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].MMRegion,1,MPI::BOOL,
                    root,105,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].PBRegion,1,MPI::BOOL,
                    root,106,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].BARegion,1,MPI::BOOL,
                    root,107,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].frozen,1,MPI::BOOL,
                    root,108,MPI_COMM_WORLD,&stat);

          // Integers
          MPI_Recv(&QMMMData[i].numTyp,1,MPI_INT,
                    root,109,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].numClass,1,MPI_INT,
                    root,110,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].id,1,MPI_INT,
                    root,111,MPI_COMM_WORLD,&stat);
          ...
          // Receive strings
          MPI_Recv(&QMMMData[i].QMTyp[0],sz1+1,MPI_CHAR,
                    root,114,MPI_COMM_WORLD,&stat);
          MPI_Recv(&QMMMData[i].MMTyp[0],sz2+1,MPI_CHAR,
                    root,115,MPI_COMM_WORLD,&stat);
          ...
          */
          ...
          /*
            cout << "Proc-" << myrank;
            cout << " finish receiving mystruct\n" << endl;
          */
          ...
          if (sz3!=0)
          {
            // In the test case sz3 was 0 so it was an giving error
            MPI_Recv(&QMMMData[i].MP[j].type,sz3+1,MPI_CHAR,
                      root,1152,MPI_COMM_WORLD,&stat);
          }
